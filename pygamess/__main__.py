...
lastlinesrun = subprocess.run(f"tail -n {args.num_last_lines} {args.output_file}", shell=True,
	stdout=subprocess.PIPE, text=True)
logging.info(f"GAMESS last {args.num_last_lines} lines: {lastlinesrun.stdout}")
yaml = YAML(typ="safe")
with open(args.emails_yml, "r") as opf:
    configuration = yaml.load(opf)

if status == 0:
	email.smtplib_email(configuration["success"]["body"].format(args.output_file, lastlinesrun.stdout), configuration["success"]["receivers"],
		configuration["success"]["subject"], configuration["smtp"])
else:
	email.smtplib_email(configuration["error"]["body"].format(args.output_file, status, lastlinesrun.stdout), configuration["error"]["receivers"],
